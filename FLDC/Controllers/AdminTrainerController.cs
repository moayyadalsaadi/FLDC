    ...
    public class AdminTrainerController : Controller
    {
        ...
        public ActionResult Create([Bind(Include = "CenterStaffId,Name,Description,Path,Code")] CenterStaff centerStaff, HttpPostedFileBase Image)
        {
            if (ModelState.IsValid)
            {
                ...
                //save  image
                string path1_1 = Path.Combine(Server.MapPath("~/ImagesOfProject/TrainerPhotos"), Image.FileName);
                string Extension1 = Path.GetExtension(path1_1);
                string path1_2 = Path.Combine(Server.MapPath("~/ImagesOfProject/TrainerPhotos"), id + Extension1);
                Image.SaveAs(path1_2);
                string Domain = ConfigurationManager.AppSettings["Domain"].ToString();
                centerStaff.Path = Domain + "/ImagesOfProject/TrainerPhotos/" + id + Extension1;                
